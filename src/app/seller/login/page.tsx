...
import { getAuth, login, getPostLoginPath } from "@/lib/auth";
...
export default function SellerLogin() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        console.log("Seller login started", { email });
        ...
        try {
            ...
            setError(err.message || "Failed to log in to Seller Portal.");
        } finally {
            setIsLoading(false);
            console.log("Seller login ended", { loading: false });
        }
    };
        ...
        <main className="relative min-h-screen flex items-center justify-center font-inter bg-[#0A0F1F] overflow-hidden text-[#FFFFFF]">
            ...
            >
                <div className="bg-[#111627]/70 backdrop-blur-2xl border border-[#4DA3FF]/20 rounded-[2.5rem] p-10 shadow-[0_20px_60px_rgba(77,163,255,0.08)] flex flex-col items-center group/card transition-all duration-500 hover:border-[#4DA3FF]/40 relative overflow-hidden">
                    ...
                    <h1 className="text-3xl font-extrabold text-[#FFFFFF] mb-2 text-center tracking-tight">Seller Portal</h1>
                    <p className="text-[#A0AEC0] text-sm text-center mb-10 font-medium">Manage properties, uploads, and inquiries.</p>
                    ...
                    {currentRole === "customer" && (
                        <div className="bg-amber-500/10 border border-amber-500/30 text-amber-500 text-sm font-semibold px-4 py-3 rounded-xl mb-6 text-center w-full relative z-10 flex items-center justify-center gap-2">
                            <AlertCircle size={18} />
                            <span>You are currently a Customer. Logout to switch.</span>
                        </div>
                    )}
                    ...
                    {error && (
                        ...
                        >
                            {error}
                            {error.includes("registered as a Customer") && (
                                <Link href="/login" className="block mt-2 text-[#7CC4FF] hover:text-white underline transition-colors">
                            ...
                            <div className="flex flex-col gap-2 relative group text-left w-full">
                                ...
                                <div className="flex justify-end p-1">
                                    <button
                                        ...
                                        className="text-xs text-[#7CC4FF] hover:text-[#FFFFFF] font-bold transition-all hover:underline"
                                    >
                                        Forgot Password?
                                    </button>
                                </div>
